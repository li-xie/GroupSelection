...
N = 100; % total num of communities within one cycle
...
max_popul = 1e5; % maximal number of cells in the community
t_bin = 0.01; % time step in the simulation
...
gH_max = 0.1; % max growth rate of H
...
M0=100; % number of M cells in a Newborn
H0=1; % number of H cells in a Newborn
...
%             
            ...
            % the time step
            gM_exp = t_bin*gM_max; %trapz(tx,g_M)*gM_max;
            gH_exp = t_bin*gH_max; %trapz(tx,g_H)*gH_max;
...
%             R(dt) = RN(end)*K_MR;
            ...
            % draw mutation effect from mu_specZeroNull, and change the value of fp
            % of both daughter cells
            if div_length > 0
                mut_multiplier1 = mu_specZeroNull(div_length).*double(rand(div_length,1) <= mut_rate)+1;
                mut_multiplier2 = mu_specZeroNull(div_length).*double(rand(div_length,1) <= mut_rate)+1;
...
%     save([folder_name2 '/newborns'],'newborns');
    ...
    newborn_selected = newborns(I(1:sel_counter));
    ...
    save([folder_name1 '/newborn_selected'],'newborn_selected');

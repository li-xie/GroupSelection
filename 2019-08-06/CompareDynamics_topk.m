...
%     Va_inter_mean = zeros(num_cycles+1, 1);
    clr = {'b','k','r'};
    for i = 1:1000%num_cycles
        fp_pre = fp0_sel_cal(i);
%                 Va_m(i) = (24*mu^2*mu_sig^4*fp_pre^4*(1-fp_pre)^2*log(1.4*g0T*B0))^(1/3);
                Va_inter_mean(i+1) = Va_m(i)*4*b0/B0;
        
%         % calculate the inter-group variance from simulation
%         fp0_all_temp = fp0_all(i, :);
%         Va_inter_mean(i+1) = var(fp0_all_temp(~isnan(fp0_all_temp)),1);

        %         % calculate the improvement through integration
        ...
        %                 
        % calculate the improvement through the appoximated equation

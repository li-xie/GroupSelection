function [fp_manu,L_manu,N_manu,L_help,N_help,Bio_M,Bio_H,R,B,P,n_genos,gc]...
    = simulateOneWell(gc,ic_fp_manu,ic_L_manu,ic_N_manu,ic_L_help,ic_N_help,ic_n_genos)
...
R_M = @(R) R ./ gc.K_MR;
B_M = @(B) B ./ gc.K_MB;
b_M = @(R,B) gc.b_Mmax; % exponential growth!
...
for tstep = 1 : gc.nsteps
    ...
    % RBFG is just a constant now:
    %RBFG_prime = @(RBFG) gc.b_Mmax;
    %RBFG_now = 0;
    %tspan = [0, gc.dt];
    %[~,RBFG] = ode23s(@(t,RBFG) RBFG_prime(RBFG), tspan, RBFG_now);
    % don't update resource and byproduct
    ...
    %int_M = RBFG(end);
    L_manu = L_manu .* exp((1 - fp_manu) * gc.dt * gc.b_Mmax);
    lgt2 = L_manu > 2;

function simulateGroupSelection_pregrowth(B0,prop_method,S,num_cycles,...
    num_wells,mu,label)
% modified from simulateGroupSelectionFn. Added a pre-growth step.
...
% pre_growth constant
prec = gc;
prec.nsteps = round(pre_time/gc.dt); % number of time steps for pregrowth

ic_fp = [fp_anc;zeros(100 - 1,1)]; % initial distribution of fp
ic_b = [1;zeros(100 - 1,1)]; % initial distribution of individual biomass
ic_n = [1 ;zeros(100 - 1,1)]; % initial number of cells
ic_n_genos = 1; % initial number of genotype
% B0 = gc.B0; % B_target
len_fp_init = length(ic_fp);
pre_rng = rng;
[pre_fp, pre_b, pre_n, ~, pre_n_genos, ~] = simulateOneWell(prec, ic_fp, ic_b, ic_n, ic_n_genos);
...
[newb_fp,newb_b,newb_n, newb_n_genos] = prop_method(...
            ...
            pre_fp, pre_b, pre_n, pre_n_genos,...
            (1:num_wells),floor(pre_b' * pre_n/B0),cw_params,len_fp_init,B0);

% save simulation conditions 
save([resultsfolder 'pregrowth'],'pre_fp', 'pre_b', 'pre_n', 'pre_n_genos', 'pre_rng')

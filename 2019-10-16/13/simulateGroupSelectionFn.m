...
save([resultsfolder 'gc-struct'],'gc');
...
if prev_cycles < 1
    ...
    load([resultsfolder 'nb' num2str(prev_cycles+1)])
    % % add 0.01 to fp
    % for i = 1 : num_wells
...
for c = 1 : num_cycles
    save_newborns(newb_fp,newb_b,newb_n,newb_n_genos,newb_pnum,...
        seeds(c), prev_cycles+c, resultsfolder);
    rng(seeds(c));
    ...
    scurr = rng;
    % save the summary of all the Adults
    savePlateSummary(wellPlate_fp, wellPlate_b, wellPlate_n,...
        wellPlate_B, wellPlate_P, wellPlate_n_genos,...
        c+prev_cycles, resultsfolder, cw_params, scurr);
    ...
    for i = 1 : Sp
        ...
        [newb_fp,newb_b,newb_n, newb_n_genos,] = prop_method(...
            ...
            wellPlate_n_genos{P_ind(i)},target_inds,floor(B_sorted(i)/B0),cw_params,B0);
    end
...
save_newborns(newb_fp,newb_b,newb_n,...
    newb_n_genos,newb_pnum, 0, c+prev_cycles+1, resultsfolder);

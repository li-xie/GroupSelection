function simulateGroupSelectionFn_offset(B0,prop_method,S,end_cycle, num_cycles,...
    num_wells,mu,label)
...
rng('shuffle');
seeds = randi(2^32-1,[num_cycles + end_cycle,1],'uint32');

if end_cycle >= 1
load([resultsfolder '/nb' num2str(end_cycle+1)])
newb_pnum = pnum;
% % add 0.01 to fp
% for i = 1 : num_wells
...
save_newborns(newb_fp,newb_b,newb_n,newb_n_genos,newb_pnum,end_cycle,...
    resultsfolder);
for gen = 1+end_cycle : num_cycles+end_cycle
    rng(seeds(gen));

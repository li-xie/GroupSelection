...
%         end
        ...
        for j = 1 : num_wells
            ...
            fp0_all(i, j) = sum(newb_fp{j}.*newb_b{j}.*newb_n{j})/max(1, B0_all(i, j));
            varfp0_all(i, j) = sum((newb_fp{j}-fp0_all(i, j)).^2.*newb_b{j}.*newb_n{j})/max(1, B0_all(i, j));
            BT_all(i, j) = sum(b{j}.*n{j});
            fpT_all(i, j) = sum(fp{j}.*b{j}.*n{j})/max(1, BT_all(i, j));
            varfpT_all(i, j) = sum((fp{j}-fpT_all(i, j)).^2.*b{j}.*n{j})/max(1, BT_all(i, j));
            P_all(i, j) = P{j}(end);
        end
...
%         'fpT_all','varfpT_all','BT_all','P_all','P_sel')
    save([num2str(f) '/' 'fp_data' num2str(c)],'fp0_all','varfp0_all','B0_all',...
        'fpT_all','varfpT_all','BT_all','P_all')
end
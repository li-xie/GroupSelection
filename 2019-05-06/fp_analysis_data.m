...
% varfp0=(6*mu^2*0.05^4.*fp0.^4.*(1-fp0).^2*log(1.4/1e-4)).^(1/3);
figure(1)
...
rhd = varfp0(1:end-1) .* (1 - fp0(1:end-1).*(1-fp0(1:end-1))*g0T)./(fp0(1:end-1).*(1-fp0(1:end-1)))...
    *sqrt(sqrt(2)/B0*H)*(1-(log(max(S,1))-1)/H)- varfp0(1:end-1).*g0T...
...
plot(log10(varfp0),'.')
hold on
plot(log10((6*mu^2*0.05^4.*fp0.^4.*(1-fp0).^2*log(1.4*B0*g0T)).^(1/3)),'o')
hold off

% scatter(varfp0,(6*mu^2*0.05^4.*fp0.^4.*(1-fp0).^2*log(1.4/1e-4)).^(1/3))